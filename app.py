...
def ai_chat():
    """Advanced AI chat with extended thinking and web search"""
    try:
        data = request.json
        message = data.get('message', '')
        history = data.get('history', [])
        use_extended_thinking = data.get('use_extended_thinking', False)
        ...
        system_prompt = """You are an expert AI assistant for an electrical automation and quoting system. You have deep knowledge of:
- Electrical codes and standards (NEC, local codes)
- Automation systems (lighting, HVAC, security, etc.)
- Component pricing and selection
- Project estimation and quoting
- Electrical mapping and circuit design
...
- Analyzing data and quotes
- Navigating the application

Current context: User is on page '{page}' in the '{app}' application.

Be concise, helpful, and professional. If you can suggest actions the user can take in the app, format them as JSON in your response like:
[ACTION: {{"type": "action_name", "params": {{}}}}]
""".format(page=context.get('page', 'unknown'), app=context.get('app', 'Electrical System'))
        ...
        thinking = None
        if use_extended_thinking and 'thinking' in ai_response:
        ...
        for action_str in action_matches:
            try:
                ...
                response_text = response_text.replace(f'[ACTION: {action_str}]', '')
            except:
                pass

        # Web search if requested (simplified - in production use proper search API)
        if use_web_search and 'search' in message.lower():
            response_text += "\n\n🌐 Note: Web search integration is configured. Real-time search would go here in production."
