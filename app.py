...
os.makedirs(PDF_EDITOR_AUTOSAVE_DIR, exist_ok=True)

@app.route('/pdf-editor')
def pdf_editor_page():
    """Render PDF editor page"""
    return render_template('quote_pdf_editor.html')

@app.route('/api/pdf-editor/templates', methods=['GET', 'POST'])
def handle_pdf_templates():
    ...
    try:
        if request.method == 'GET':
            ...
            save_json_file(PDF_TEMPLATES_FILE, templates)
...
@app.route('/api/pdf-editor/templates/<template_id>', methods=['GET', 'PUT', 'DELETE'])
...
@app.route('/api/pdf-editor/templates/last', methods=['GET'])
...
def save_pdf_editor_to_crm():
    ...
    try:
        ...
        # Parse canvas data to extract CRM updates
        ...
        # Process canvas objects to update CRM data
        import re
        updates_made = []
        ...
        if 'objects' in canvas_obj:
            for obj in canvas_obj['objects']:
                if 'dataBinding' in obj:
                    ...
                    # Handle different types of data bindings
                    if binding_type == 'text':
                        # Extract text content
                        text_content = obj.get('text', '')

                        # Update quote fields based on binding
                        if field_key and text_content:
                            ...
                            if numbers:
                                ...
                                # Update corresponding CRM field
                            ...
                            # Handle text fields (title, description, etc.)
                        ...
                        # Handle table data bindings for cost centres, labour, stock items
                        # These are more complex and would need specialized parsing
                        # For now, we'll log that table data was detected
                        logger.info(f"Table binding detected for field: {field_key}")

        # Recalculate quote amount if pricing changed
